...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    device = DeviceInfo(
        identifiers={(DOMAIN, coordinator.serial or "unknown")},
        name="ChargeSentry",
...
class BaseCS(CoordinatorEntity[ChargeSentryDataUpdateCoordinator], SensorEntity):
    ...
    def available(self) -> bool:
        # Only available when last update succeeded and we have coordinator data
...
class ChargerPowerSensor(BaseCS):
    ...
    def native_value(self):
        live = self._live
        return None if not isinstance(live, dict) else live.get("power_w")
...
class ChargerStatusSensor(BaseCS):
    ...
    def native_value(self):
        live = self._live
        return None if not isinstance(live, dict) else live.get("status")
...
class ChargerEnergySensor(BaseCS):
    ...
    def native_value(self):
        energy = self._energy
        return None if not isinstance(energy, dict) else energy.get("lifetime_kwh")
